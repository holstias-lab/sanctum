...
(function(window) {
  ...
  class ProperPiperTTS {
    constructor(voiceId = 'en_US-hfc_female-medium') {
      ...
      // NOTE: this class is constructed asynchronously (well after this
      // script tag finishes executing), so document.currentScript is
      // always null here — it cannot be used to locate our own files.
      // Hardcode the vendor path instead.
      const baseUrl = 'assets/vendor/piper';
      this.voiceModelPath = baseUrl + `/voices/${voiceId}.onnx`;
      this.voiceConfigPath = baseUrl + `/voices/${voiceId}.onnx.json`;
      ...
      this.availableVoices = {
        ...
        // Sanctum-added voices
        'ar_JO-kareem-medium': 'Arabic (Jordan, Kareem) - Medium',
        'zh_CN-huayan-medium': 'Mandarin (Huayan) - Medium'
      };
    }
    ...
    async loadPhonemizer() {
      ...
      const response = await fetch(this.baseUrl + '/piper-o91UDS6e.js');
      if (!response.ok) {
        throw new Error(`Failed to load phonemizer module: ${response.status}`);
      }
    ...
    async initPhonemizer() {
      ...
      const wasmPaths = {
        piperWasm: this.baseUrl + '/piper_phonemize.wasm',
        piperData: this.baseUrl + '/piper_phonemize.data'
      };
    ...
    async changeVoice(voiceId) {
      ...
      // Update voice paths
      this.voiceId = voiceId;
      this.voiceModelPath = this.baseUrl + `/voices/${voiceId}.onnx`;
      this.voiceConfigPath = this.baseUrl + `/voices/${voiceId}.onnx.json`;
